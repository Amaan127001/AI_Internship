...
async function getCity(input, setCities, index) {
  try {
    ...
    if (/^\d+$/.test(query)) {
      ...
      const zipUrl = `${proxy}https://api.openweathermap.org/geo/1.0/zip?zip=${query},${country}&appid=${API_KEY}`;
      ...
      const directUrl = `${proxy}https://api.openweathermap.org/geo/1.0/direct?q=${encodeURIComponent(query)}&limit=5&appid=${API_KEY}`;
    ...
    const currentUrl = `${proxy}https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=metric`;
    // …and the 5-day forecast data.
    const forecastUrl = `${proxy}https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${API_KEY}&units=metric`;
